...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 17 16:40:31 2025
...
def main():
    ...
    T1_0_list = np.array((800, 1000, 1250, 1500, 2000))  # vary T1_0 from short to long
    ...
    for repeat in range(0, 10000):
        ...
        for T1_0_val in T1_0_list:
            F_p_array = np.ones(n_samples) * F_p  # Fixed Fp
            v_e_array = np.ones(n_samples) * v_e  # Fixed ve
            v_p_array = np.ones(n_samples) * v_p  # Fixed vp
            T1_0_array = np.ones(n_samples) * T1_0_val  # Varying T1_0
            init_params_row = np.column_stack((F_p_array, PS, v_e_array, v_p_array, tau_a))
            init_params_all.append(init_params_row)
            T1_0_all_grid.append(T1_0_array)
        
        init_params = np.vstack(init_params_all)  # Stack all rows
        T1_0_all = np.hstack(T1_0_all_grid)  # Stack all T1_0 values
        ...
        for t1_idx in range(n_samples):
            for ps_idx in range(n_samples):
                params_idx = t1_idx * n_samples + ps_idx  # Linear index into results
                ...
                best_fits[t1_idx, ps_idx] = {'NLM': 0, 'PTM': 1, 'ETM': 2}[best_model]
                best_Ktrans[t1_idx, ps_idx] = Ktrans_fit[best_model]
                best_vp[t1_idx, ps_idx] = vp_fit[best_model]
                if best_model == 'ETM':
                    best_ve[t1_idx, ps_idx] = ve_fit[best_model]
                else:
                    best_ve[t1_idx, ps_idx] = np.nan  # NLM and PTM don't have ve
                PTM_Ktrans[t1_idx, ps_idx] = Ktrans_fit['PTM']
                ETM_Ktrans[t1_idx, ps_idx] = Ktrans_fit['ETM']
                NLM_vp[t1_idx, ps_idx] = vp_fit['NLM']
                PTM_vp[t1_idx, ps_idx] = vp_fit['PTM']
                ETM_vp[t1_idx, ps_idx] = vp_fit['ETM']
                ETM_ve[t1_idx, ps_idx] = ve_fit['ETM']
        
        # Save results
        base_path = f'/Users/user/Documents/Python_scripts/MODELLING_SIMULATIONS/Paper_Draft_2/varT1cut40_{tissue}/'
        if os.path.isdir(base_path) == False:
            os.mkdir(base_path)
            
        # Create input parameter grids for saving
        ground_truth_PS = np.tile(PS, (n_samples, 1))
        ground_truth_T1 = np.tile(T1_0_list, (n_samples, 1)).T
        best_fits_showing = best_fits + 1
        
        # Save all results
        np.savetxt(f'{base_path}ground_truth_PS.txt', ground_truth_PS, fmt='%f')
        np.savetxt(f'{base_path}ground_truth_T1.txt', ground_truth_T1, fmt='%f')

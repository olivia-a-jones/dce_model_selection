...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 17 15:26:53 2025
...
def main():
    ...
    F_p_list = np.array((0.1, 0.3, 0.6, 0.8, 1.2))  # vary Fp from 0.1 (hypoperfused) to 0.8 (highly perfused)
    ...
    for repeat in range(0, 10000):
        ...
        init_params_all = []
        for F_p in F_p_list:
            F_p_array = np.ones(n_samples) * F_p
            init_params_row = np.column_stack((F_p_array, PS, v_e, v_p, tau_a))
        ...
        for fp_idx in range(n_samples):
            for ps_idx in range(n_samples):
                params_idx = fp_idx * n_samples + ps_idx  # Linear index into results
                ...
                best_fits[fp_idx, ps_idx] = {'NLM': 0, 'PTM': 1, 'ETM': 2}[best_model]
                best_Ktrans[fp_idx, ps_idx] = Ktrans_fit[best_model]
                best_vp[fp_idx, ps_idx] = vp_fit[best_model]
                if best_model == 'ETM':
                    best_ve[fp_idx, ps_idx] = ve_fit[best_model]
                else:
                    best_ve[fp_idx, ps_idx] = np.nan  # NLM and PTM don't have ve
                PTM_Ktrans[fp_idx, ps_idx] = Ktrans_fit['PTM']
                ETM_Ktrans[fp_idx, ps_idx] = Ktrans_fit['ETM']
                NLM_vp[fp_idx, ps_idx] = vp_fit['NLM']
                PTM_vp[fp_idx, ps_idx] = vp_fit['PTM']
                ETM_vp[fp_idx, ps_idx] = vp_fit['ETM']
                ETM_ve[fp_idx, ps_idx] = ve_fit['ETM']
        
        # Save results
        base_path = f'/Users/user/Documents/Python_scripts/MODELLING_SIMULATIONS/Paper_Draft_2/varFpcut40_{tissue}/'
        if os.path.isdir(base_path) == False:
            os.mkdir(base_path)
            
        # Create input parameter grids for saving
        ground_truth_PS = np.tile(PS, (n_samples, 1))
        ground_truth_Fp = np.tile(F_p_list, (n_samples, 1)).T
        best_fits_showing = best_fits + 1
        
        # Save all results
        np.savetxt(f'{base_path}ground_truth_PS.txt', ground_truth_PS, fmt='%f')
        np.savetxt(f'{base_path}ground_truth_Fp.txt', ground_truth_Fp, fmt='%f')

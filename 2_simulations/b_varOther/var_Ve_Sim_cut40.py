...
# -*- coding: utf-8 -*-
"""
Created on Wed Sep 17 16:37:40 2025
...
def main():
    ...
    v_e_list = np.array((0.05, 0.1, 0.15, 0.2, 0.25))  # vary v_e from 5% (small) to 25% (large)
    ...
    for repeat in range(0, 10000):
        ...
        for v_e in v_e_list:
            ...
            v_e_array = np.ones(n_samples) * v_e  # Varying ve
        ...
        for ve_idx in range(n_samples):
            for ps_idx in range(n_samples):
                params_idx = ve_idx * n_samples + ps_idx  # Linear index into results
                ...
                best_fits[ve_idx, ps_idx] = {'NLM': 0, 'PTM': 1, 'ETM': 2}[best_model]
                best_Ktrans[ve_idx, ps_idx] = Ktrans_fit[best_model]
                best_vp[ve_idx, ps_idx] = vp_fit[best_model]
                if best_model == 'ETM':
                    best_ve[ve_idx, ps_idx] = ve_fit[best_model]
                else:
                    best_ve[ve_idx, ps_idx] = np.nan  # NLM and PTM don't have ve
                PTM_Ktrans[ve_idx, ps_idx] = Ktrans_fit['PTM']
                ETM_Ktrans[ve_idx, ps_idx] = Ktrans_fit['ETM']
                NLM_vp[ve_idx, ps_idx] = vp_fit['NLM']
                PTM_vp[ve_idx, ps_idx] = vp_fit['PTM']
                ETM_vp[ve_idx, ps_idx] = vp_fit['ETM']
                ETM_ve[ve_idx, ps_idx] = ve_fit['ETM']
        
        # Save results
        base_path = f'/Users/user/Documents/Python_scripts/MODELLING_SIMULATIONS/Paper_Draft_2/varVecut40_{tissue}/'
        if os.path.isdir(base_path) == False:
            os.mkdir(base_path)
            
        # Create input parameter grids for saving
        ground_truth_PS = np.tile(PS, (n_samples, 1))
        ground_truth_ve = np.tile(v_e_list, (n_samples, 1)).T
        best_fits_showing = best_fits + 1
        
        # Save all results
        np.savetxt(f'{base_path}ground_truth_PS.txt', ground_truth_PS, fmt='%f')
        np.savetxt(f'{base_path}ground_truth_ve.txt', ground_truth_ve, fmt='%f')
